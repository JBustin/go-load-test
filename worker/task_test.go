...
func Test_TaskMethods(t *testing.T) {
	...
	assert.Equal(t, 0*time.Millisecond, task.Duration("request"), "should return request duration")
	assert.Equal(t, 0*time.Millisecond, task.Duration("connect"), "should return connect duration")
